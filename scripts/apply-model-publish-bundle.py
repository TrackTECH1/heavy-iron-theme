...
def build_fields(entry: dict) -> list[dict]:
    fields: list[dict] = []
    pts = (entry.get("primary_track_size") or "").strip()
    if pts:
        fields.append({"key": "primary_track_size", "value": pts})
    track = entry.get("track_product_gids") or []
    uc = entry.get("uc_product_gids") or []
    if track:
        fields.append({"key": "track_products", "value": json.dumps(sorted(track))})
    if uc:
...
def main() -> int:
    ...
    for entry in models:
        ...
        if args.only_changed and apply:
            ...
            new_track = sorted(entry.get("track_product_gids") or [])
        ...
        label = (
            ...
            f"tracks={len(entry.get('track_product_gids') or [])} "
